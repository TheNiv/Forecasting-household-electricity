...
def resample_data(df,rule='H'):
	...
	:param str rule: the time period that seperate each timestep, can be DateOffset, Timedelta or a string.
...
def new_scale_data(df,method='MinMax',in_place = False):
    ...
    for column in df.columns:
       #print(df[column].to_numpy().reshape(-1, 1).shape)
       scaler = preprocessing.MinMaxScaler()
...
def split_to_windows(sequences,to_predict, window_size=1):
    ...
    Creates an array of windows from the dataset and an array of outputs.
